	...
	if (SDL_PollEvent(&event))
	{
		...
		switch (gbuffer[0])
		{
			...
			case 'M':
    			...
    			goEnabled = (gId == currentPlayer); // activer bouton go uniquement pour le joueur courant
				break;
			...
			case 'V':
				...
				if (gbuffer[2] == 'O') {
					int count;
					sscanf(gbuffer, "R O %d", &count);
					printf("[INFO] %d joueur(s) possèdent ce symbole.\n", count);
				} else if (gbuffer[2] == 'S') {
					int quantite;
					sscanf(gbuffer, "R S %d", &quantite);
					printf("[INFO] Le joueur ciblé possède %d carte(s) avec ce symbole.\n", quantite);
				}
				break;
			...
			case 'W':
				{
					int idGagnant;
					sscanf(gbuffer, "W %d", &idGagnant);
					if (idGagnant == gId)
						printf("[VICTOIRE] Tu as trouvé le coupable ! Bravo Sherlock 🕵️‍♂️\n");
					else
						printf("[FIN] Le joueur %s a gagné la partie.\n", gNames[idGagnant]);
					quit = 1; // quitte SDL
				}
				break;
			...
			case 'X':
				{
					int idOut;
					sscanf(gbuffer, "X %d", &idOut);
					if (idOut == gId)
						printf("[ELIMINATION] Mauvaise accusation... tu es éliminé 😢\n");
					else
						printf("[INFO] Le joueur %s a été éliminé.\n", gNames[idOut]);
					if (idOut == gId) quit = 1;
				}
				break;
