        	...
        	switch (buffer[0])
        	{
                                ...
                                if (nbClients==4)
				{
					...
					// RAJOUTER DU CODE ICI
					    char messageCartes_0 [256];
    				sprintf(messageCartes_0, "D %d %d %d", deck[0], deck[ 1], deck[ 2]);
    				sendMessageToClient(tcpClients[0].ipAddress, tcpClients[0].port, messageCartes_0);
					// On envoie ses cartes au joueur 1, ainsi que la ligne qui lui correspond dans tableCartes
					// RAJOUTER DU CODE ICI
					char messageCartes_1[256];
					sprintf(messageCartes_1, "D %d %d %d", deck[3], deck[4], deck[5]);
					sendMessageToClient(tcpClients[1].ipAddress, tcpClients[1].port, messageCartes_1);
					// On envoie ses cartes au joueur 2, ainsi que la ligne qui lui correspond dans tableCartes
					// RAJOUTER DU CODE ICI
					// On envoie ses cartes au joueur 2, ainsi que la ligne qui lui correspond dans tableCartes
					char messageCartes_2[256];
					sprintf(messageCartes_2, "D %d %d %d", deck[6], deck[7], deck[8]);
					sendMessageToClient(tcpClients[2].ipAddress, tcpClients[2].port, messageCartes_2);

					// On envoie ses cartes au joueur 3, ainsi que la ligne qui lui correspond dans tableCartes
					// RAJOUTER DU CODE ICI
					// On envoie ses cartes au joueur 3, ainsi que la ligne qui lui correspond dans tableCartes
					char messageCartes_3[256];
					sprintf(messageCartes_3, "D %d %d %d", deck[9], deck[10], deck[11]);
					sendMessageToClient(tcpClients[3].ipAddress, tcpClients[3].port, messageCartes_3);

					// RAJOUTER DU CODE ICI
					// On envoie la table des symboles à tout le monde
					for (int i = 0; i < 4; i++) {
    				for (int j = 0; j < 8; j++) {
        					char msg[256];
        					sprintf(msg, "V %d %d %d", i, j, tableCartes[i][j]);
	...
	else if (fsmServer==1)
	{
		switch (buffer[0])
		{
				...
				// RAJOUTER DU CODE ICI
				{
					...
					if (deck[12] == suspect) {
						// Bonne accusation
						sprintf(reply, "W %d", id); // W pour Win
						broadcastMessage(reply);
						fsmServer = 2; // Partie terminée
					} else {
						// Mauvaise accusation
						sprintf(reply, "X %d", id); // exclue "x"
						broadcastMessage(reply);
						strcpy(tcpClients[id].name, "-"); // marquer comme éliminé

				
						// Passer au joueur suivant
						do {
							joueurCourant = (joueurCourant + 1) % 4;
						} while (strcmp(tcpClients[joueurCourant].name, "-") == 0); // Ignore les joueurs exclus
				...
				// RAJOUTER DU CODE ICI
				{
					int id, symbole, count = 0;
					sscanf(buffer, "O %d %d", &id, &symbole);
				
					for (int i = 0; i < 4; i++) {
						if (i == id) continue; // Ne compte pas lui-même
						if (tableCartes[i][symbole] > 0)
							count++;
					}
				
					sprintf(reply, "R O %d", count); // R pour Réponse, O pour type
			...
			case 'S':
				// RAJOUTER DU CODE ICI
				{
					int id, cible, symbole;
					sscanf(buffer, "S %d %d %d", &id, &cible, &symbole);
				
					int quantite = tableCartes[cible][symbole];
				
					sprintf(reply, "R S %d", quantite); // Réponse ciblée
					sendMessageToClient(tcpClients[id].ipAddress, tcpClients[id].port, reply);
					...
					sprintf(reply, "M %d", joueurCourant);
					broadcastMessage(reply);
					printf("DEBUG: Sent 'M %d' to all clients\n", joueurCourant);
